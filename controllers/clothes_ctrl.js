...
async function upload(req, res) {
  ...
  try {
    let clothes = await clothesService.getClothesbyID(id);
    if (!clothes) {
      return res.status(404).json({ error: "Clothes not found" });
    }

    const imageUrl = await clothesService.upload(file);
    clothes = await clothesService.updateClothes(id, imageUrl);
...
async function getStylesByClothes(req, res) {
  try {
    ...
    if (styleIds.length === 0) {
      return res
        .status(404)
        .json({ error: "No styles found for this clothes" });
    }
...
async function deleteClothes(req, res) {
  ...
  try {
    ...
    await clothesService.deleteClothes(id);
